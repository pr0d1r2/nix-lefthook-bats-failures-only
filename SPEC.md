...
| B3 | 2026-08-12 | The pinned set-and-setting actionlint check passes a string path prefix to the current Nix source filtering API, so requesting the actions fragment makes `nix flake check` fail during evaluation. | Do not request the incompatible actions fragment or retain its generated hook commands; GitHub Actions validation remains provided by the shared guardrails workflow. |
